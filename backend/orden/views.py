...
def confirmar_pago(request):
    ...
    return JsonResponse(
        {
            "success": True,
            "redirect_url": reverse("exito_orden", kwargs={"ordenID": orden.ordenID}),
        }
    )


@login_required(login_url="login")
def exito_orden(request, ordenID):
    orden = get_object_or_404(
        Orden.objects.prefetch_related("items__product").select_related("direccion_envio", "promo_codigo"),
        ordenID=ordenID,
        user=request.user,
        status=OrdenStatus.COMPLETED,
    )
    return render(request, "orden/exito.html", {"orden": orden})
